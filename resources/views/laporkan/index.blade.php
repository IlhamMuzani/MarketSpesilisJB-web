...
<!-- Content Header (Page header) -->
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Tabel Pengaduan </h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
    ...
    <section class="content">
      ...
      <div class="card">
              ...
              <div class="card">
              <div class="card-header">
              <form method="get" action="{{route('laporkan.index')}}">
                <h3 class="card-title"> 
              ...
              <div class="card-body table-responsive p-0">
                <table class="table table-hover text-nowrap">
                  ...
                  @foreach($listlaporkan as $key => $data)
                    <tr>
                      <td>{{ $listlaporkan->firstItem() + $key }}</td>
                      <!-- <td>{{ $data->kd_user }}</td> -->
                      <td>{{ $data->user['username'] }}</td>
                      <td>{{ $data->produk['nama_toko'] }}</td>
                      <td>{{ $data->produk['jenis_pembuatan'] }}</td>
                      <td>
                      <img class="img-thumbnail" src="{{ asset('storage/uploads/'.$data->gambar) }}" width="100px">
                      </td>  
                      <td>
                      <form method="post" action="{{ route('laporkan.destroy', $data->id) }}" onsubmit="return confirm('Apakah anda yakin akan menghapus data ini ?')">
                                @csrf
                                {{ method_field('DELETE') }}
                                ...
                                <a class="btn btn-primary" href="{{ route('laporkan.show', $data->id)}}"> <i class="fas fa-info-circle"></i></a>
                                </form>
                      </td>
                    </tr>
                  @endforeach
                ...
                {{ $listlaporkan->appends(Request::all())->links() }}
              </div>
            </div>
           </div>
          </div>
